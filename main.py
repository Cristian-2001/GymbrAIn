...
if __name__ == "__main__":
    ...
    user_poses = deep_learning.estimate_poses(video_path)
    ...
    data_luca = utils.load_json("poses_finale.json")

    # Retrieve the video paths for the example exercise
    luca_video_path, luca_video_path_720, exercise_name = video_processing.retrieve_video(user_poses, data_luca)
    ...
    min_vel_frames = geometry.find_min_vel(user_poses, keypoint=keypoint_to_use_vel[ex_index], th=thresholds[ex_index])

    # Find the corresponding frames in the example video
    frame_list = utils.find_frames(luca_video_path, min_vel_frames)

    # Append the poses with minimum velocity to the list
    for index in min_vel_frames:
        stationary_user_poses.append(user_poses[index])
    ...
    poses_luca = utils.find_same_poses(stationary_user_poses, poses_luca)

    # Compare the angles of the user's poses with the example poses
    utils.check_correctness(stationary_user_poses, poses_luca, ex_index)
